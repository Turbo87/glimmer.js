...
function descriptorForTrackedComputedProperty(target: any, key: any, descriptor: PropertyDescriptor, dependencies: string[]): PropertyDescriptor {
  ...
  return {
    ...
    configurable: false,
    get: descriptor.get,
    set: function(this: typeof target) {
      metaFor(this).dirtyableTagFor(key).inner.dirty();
      (descriptor.set as any).apply(this, arguments);
      propertyDidChange();
    }
  };
}
...
function installDevModeErrorInterceptor(obj: object, key: string, throwError: UntrackedPropertyErrorThrower) {
  let target = obj;
  let maybeDescriptor: PropertyDescriptor | null = null;
  ...
  while (target) {
    maybeDescriptor = Object.getOwnPropertyDescriptor(target, key);
    if (maybeDescriptor) { break; }
